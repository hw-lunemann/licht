...
impl Backlight {
    ...
    fn change_brightness(&mut self, action: Action, step: usize, mode: Mode) -> anyhow::Result<()> {
        ...
        let new_brightness = match mode {
            ...
            Mode::MaxRelativeQuadratic { exponent } => {
...
fn main() -> anyhow::Result<()> {
    ...
    if let Mode::MaxRelativeQuadratic { .. } = cli.mode {
        cli.mode = Mode::MaxRelativeQuadratic { exponent: cli.exponent };
    }
    ...
    backlight.change_brightness(cli.action, cli.step, cli.mode)?;
    if cli.verbose {
        print!("{}", &backlight.get_percent());
    }
